...
import { Sparkles, Heart, Gift, Send, X, MessageCircle, ChevronRight, ChevronLeft, Volume2, VolumeX, ArrowLeft } from 'lucide-react';
...
const App = () => {
  ...
  const storyContent = [
    ...
    {
      title: "Gratidão e Encanto",
      text: "Obrigada por fazer parte da minha história, e por confiar sua pele e um pedacinho da sua vida a mim. Use esse presente como um abraço.",
      icon: <Heart className="w-10 h-10 text-[#FFD1D1]" />,
      subtext: "Com carinho, gratidão e encanto.",
      footer: "Sua fada da pele."
    }
  ];
  ...
  const handleVoucherRequest = (e) => {
    ...
    const nomeCliente = formData.nome || 'Paciente';
    const textoMensagem = `Olá! Sou a ${nomeCliente}. Acabei de ler a minha carta mágica e gostaria de agendar o meu momento de autocuidado! ❤️❤️`;

    // Codifica a mensagem
    const msgFinal = encodeURIComponent(textoMensagem);

    // Link CORRETO usando wa.me (mais confiável que api.whatsapp.com)
    const waLink = `https://wa.me/5549988429631?text=${msgFinal}`;
    
    // Abrir imediatamente no clique do usuário para funcionar no iOS Safari
    const whatsappWindow = window.open(waLink, '_blank');
    
    // Verificar se o popup foi bloqueado
    if (!whatsappWindow) {
      // Se foi bloqueado, exibir mensagem ou tentar alternativa
      alert('Por favor, permita popups para ser redirecionado ao WhatsApp');
    }
    
    // Pequeno delay para a animação de sucesso aparecer antes de resetar o formulário
    setTimeout(() => {
      // Voltar para o começo e resetar o formulário
      setStep('opening');
      setShowModal(false);
    ...
    <Routes>
      <Route path="/" element={
        <div className="min-h-screen w-full bg-[#FFF9F9] text-[#7A6B6B] selection:bg-[#E8D5D5] overflow-x-hidden relative flex flex-col items-center justify-center font-sans">
          ...
          <div className="w-full max-w-5xl px-6 relative z-10 flex flex-col items-center pt-4 sm:pt-6">
            ...
            {step === 'story' && (
              <div className="flex flex-col items-center animate-fade-in w-full max-w-3xl py-2 sm:py-4 min-h-[400px] sm:min-h-[500px] justify-between">
                ...
                <div key={storySubStep} className="text-center space-y-6 sm:space-y-8 animate-fade-in-up w-full px-4">
                  ...
                  <div>
                    ...
                    {storyContent[storySubStep].subtext && (
                      <>
                        <div className="my-4 sm:my-6"></div>
                        <p className="text-base sm:text-lg md:text-xl leading-relaxed font-light opacity-85 italic">
                          {storyContent[storySubStep].subtext}
                        </p>
                      </>
                    )}
                    {storyContent[storySubStep].footer && (
                      <>
                        <div className="my-4 sm:my-6"></div>
                        <p className="text-base sm:text-lg md:text-xl leading-relaxed font-light opacity-85 italic">
                          {storyContent[storySubStep].footer}
                ...
                {formSubmitted ? (
                  ...
                  <div className="animate-fade-in-up">
                    ...
                    <form onSubmit={handleVoucherRequest} className="space-y-5">
                      <input required type="text" placeholder="Seu nome Completo" className="w-full px-7 py-5 bg-[#FFF9F9] border-none rounded-2xl outline-none focus:ring-1 focus:ring-[#FFD1D1] text-sm" value={formData.nome} onChange={(e) => setFormData({ ...formData, nome: e.target.value })} />
          ...
          {step !== 'gift' && (
            <a 
              href="https://wa.me/5549988429631?text=Acabei%20de%20ler%20a%20minha%20carta%20m%C3%A1gica%20e%20quero%20garantir%20o%20meu%20presente%20para%202026.%20%E2%9D%A4%EF%B8%8F%E2%9D%A4%EF%B8%8F"
              target="_blank"
              rel="noopener noreferrer"
              className="fixed bottom-6 sm:bottom-10 right-6 sm:right-10 z-[90] bg-[#25D366] text-white p-4 rounded-full shadow-2xl hover:scale-110 active:scale-95 transition-all flex items-center justify-center"
            >
